...
router.get("/productos", requireAuth, async (req, res) => {
  try {
    ...
    const result = await pool.query(
      `SELECT p.*, c.nombre as categoria, cp.nombre as categoria_padre
       FROM productos p
...
router.get("/tienda-config", requireAuth, async (req, res) => {
  try {
    ...
    const result = await pool.query(
      `SELECT c.nombre, c.slug, c.whatsapp, c.plan, t.logo_url
FROM comerciantes c 
LEFT JOIN tiendas t ON c.id = t.comerciante_id
WHERE c.id = $1`,
      [comerciante_id],
...
router.put("/tienda-config", requireAuth, async (req, res) => {
  try {
    const { comerciante_id } = req;
    const { nombre, whatsapp, logo_url } = req.body;
    ...
    await pool.query(
      "UPDATE tiendas SET nombre = $1, logo_url = $2 WHERE comerciante_id = $3",
      [nombre, logo_url || null, comerciante_id],
    );
...
router.get("/categorias", requireAuth, async (req, res) => {
  try {
    ...
    const result = await pool.query(
      `SELECT c.*, cp.nombre as padre_nombre, COUNT(p.id) as productos_count 
       FROM categorias c 
       LEFT JOIN categorias cp ON c.padre_id = cp.id
       LEFT JOIN productos p ON c.id = p.categoria_id 
       WHERE c.comerciante_id = $1 AND c.activa = true
       GROUP BY c.id, cp.nombre
       ORDER BY c.padre_id NULLS FIRST, c.nombre ASC`,
      [comerciante_id],
    );
...
router.post("/categorias", requireAuth, async (req, res) => {
  try {
    ...
    const result = await pool.query(
      "INSERT INTO categorias (nombre, descripcion, comerciante_id, padre_id) VALUES ($1, $2, $3, $4) RETURNING *",
      [nombre.trim(), descripcion?.trim(), comerciante_id, padre_id || null],
    );
...
router.put("/categorias/:id", requireAuth, async (req, res) => {
  try {
    ...
    const result = await pool.query(
      "UPDATE categorias SET nombre=$1, descripcion=$2, padre_id=$3 WHERE id=$4 AND comerciante_id=$5 RETURNING *",
      [
        nombre.trim(),
        descripcion?.trim(),
        padre_id || null,
...
// ===================
// MIGRACIONES - Subcategorías y Logo
// ===================

// GET /api/admin/migrar-subcategorias - Agregar columna padre_id a categorias
router.get("/migrar-subcategorias", requireAuth, async (req, res) => {
  try {
    await pool.query(`
      ALTER TABLE categorias 
      ADD COLUMN IF NOT EXISTS padre_id INTEGER REFERENCES categorias(id) ON DELETE SET NULL
    `);

    res.json({
      success: true,
      message: "Columna padre_id agregada correctamente a categorias",
    });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
// GET /api/admin/migrar-logo - Agregar columna logo_url a tiendas
router.get("/migrar-logo", requireAuth, async (req, res) => {
  try {
    await pool.query(`
      ALTER TABLE tiendas 
    ...
    res.json({
      success: true,
      message: "Columna logo_url agregada correctamente a tiendas",
    });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
